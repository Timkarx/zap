...
M.push = function()
    ...
    if not M.histories[winid] then
        M.histories[winid] = {index = 0, bufs = {}}
    end
...
M.setup = function(opts)
    ...
    local wilfred_denton_history_nvim =
        vim.api.nvim_create_augroup(
        "WILFRED_DENTON_HISTORY_NVIM",
        {
            clear = true
        }
    )
    ...
    vim.api.nvim_create_autocmd(
        "FileType",
        {
            ...
            callback = function(event)
                if IGNORES[vim.bo[event.buf].filetype] then
                    return
                end

                vim.keymap.set(
                    "n",
                    opts.keybinds.back or "<leader>bb",
                    M.back,
                    {
                        buffer = event.buf
                    }
                ...
                vim.keymap.set(
                    "n",
                    opts.keybinds.forward or "<leader>bf",
                    M.forward,
                    {
                        buffer = event.buf
                    }
                ...
                vim.keymap.set(
                    "n",
                    opts.keybinds.view or "<leader>bv",
                    M.toggle_popup,
                    {
                        buffer = event.buf
                    }
                ...
                vim.keymap.set(
                    "n",
                    opts.keybinds.clear_history or "<leader>c",
                    M.clear_history,
                    {
                        buffer = event.buf
                    }
